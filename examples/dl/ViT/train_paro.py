    ...
    def __getitem__(self, idx):
        ...
        if dw.file.endswith(".h5"):
            with h5py.File(dw.file, "r") as f:
                key = list(f.keys())[0]
                dataset = f[key]
                # Check if dataset is 1D or more
                if dataset.ndim == 1:
...
def train_model():
    ...
    folder = Path(hp.LOG_DL_PATH, f"paro_{timestamp}") 
...
def create_train_val_test_split():
    dataset_path = Path(hp.DATASET_PATH, f"dataset_af_combined_ecg_{cfg.WINDOW_SIZE}.csv")
