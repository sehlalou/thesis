...
class VisionTransformer(nn.Module):
    def __init__(self, config: ViTModelConfig):
        ...
        self.patch_embed = PatchEmbedding(config, in_channels=1)
...
class CNN_ViT_Hybrid(nn.Module):
    def __init__(self, config: ViTModelConfig):
        ...
        self.cnn = nn.Sequential(
            nn.Conv1d(1, 64, kernel_size=5, stride=1, padding=2),  # 1 input channel for single lead
            nn.ReLU(),
        ...
        num_patches = (config.input_size // 8) // config.patch_size  # (4096//8) // 64 = 512//64 = 8
        # ViT specific layers
        self.patch_embed = PatchEmbedding(config, in_channels=256)  # Use the output channels of CNN
    ...
    def forward(self, x):
        ...
        x = self.cnn(x)  # (batch, 256 channels, length)
        ...
        cls_output = x[:, 0]  # Use the classification token output
        logits = self.classifier(cls_output)
