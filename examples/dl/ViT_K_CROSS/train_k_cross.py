...
def load_dataset():
    ...
    df = df[:100000]  # sous-échantillonnage pour l'exemple
    return df
...
def train_fold(fold_data, fold_num):
    ...
    for epoch in range(cfg.EPOCH):
        ...
        # Calcul des métriques sur le train set incluant l'AUROC
        ...
        print(f"Fold {fold_num} Epoch {epoch + 1} : train loss {train_loss:.4f}, train accuracy {train_metrics['accuracy']:.4f}, train val_auc {train_metrics['roc_auc']:.4f}")
...
def train_model_cv_and_final():
    ...
    # 3. Entraîner le modèle final sur l'ensemble du train set et évaluer sur le test set
    print("\nEntraînement du modèle final sur l'ensemble du train set...")
    ...
    # Option : réserver 10% du train set pour la validation finale
    train_df_final, val_df_final = train_test_split(train_df, test_size=0.1, random_state=cfg.RANDOM_SEED, stratify=train_df["label"])
    train_dataset_final = DetectionDataset(train_df_final)
    val_dataset_final = DetectionDataset(val_df_final)
    ...
    for epoch in range(cfg.EPOCH):
        final_model.train()
        train_losses = []
        for x, y in tqdm(train_loader_final, desc=f"Final Model Epoch {epoch+1}"):
            x = x.to(device)
            y = y.to(device)
            optimizer.zero_grad()
        ...
        train_loss = np.mean(train_losses)
        # Calcul des métriques sur le train set incluant l'AUROC pour le modèle final
        train_metrics = estimate_metrics(final_model, train_loader_final, device)
        val_loss = estimate_loss(final_model, device, val_loader_final, criterion)
        val_metrics = estimate_metrics(final_model, val_loader_final, device)
        ...
        print(f"Final Model Epoch {epoch + 1} : val loss {val_loss:.4f}, val accuracy {val_metrics['accuracy']:.4f}, val roc_auc {val_metrics['roc_auc']:.4f}, "
              f"val sensitivity {val_metrics['sensitivity']:.4f}, val specificity {val_metrics['specificity']:.4f}")
        ...
        if patience_counter >= cfg.PATIENCE:
            print(f"Early stopping du modèle final à l'epoch {epoch + 1}")
